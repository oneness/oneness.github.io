...
steps missing (or did not result it what is expected) or not
necessary, please drop me a line via k tuman at acm dot org so I can
address and update this blog if needed.
...
- Your wifi most likely will just work. In case if you get unlucky,
  ...
  #+begin_src shell
    ...
    doas echo "join wifiname wpakey password" >> /etc/hostname.<wificardnmae>
    doas echo "dhcp" >> /etc/hostname.<wificardnmae>
